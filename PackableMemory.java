...
class PackableMemory {
	...
	// Modified to provide own byte[] entry point
	// same code as provided pack
	public static void modifiedPack(byte[] mem, int val, int loc) {
		final int MASK = 0xff;
		for (int i = 3; i >= 0; i--) {
			mem[loc + i] = (byte) (val & MASK);
			val = val >> 8;
	...
	// Modified to provide own byte[] entry point
	// same code as provided unpack
	public static int modifiedUnpack(byte[] mem, int loc) {
		final int MASK = 0xff;
		int v = (int) mem[loc] & MASK;
		for (int i = 1; i < 4; i++) {
			v = v << 8;
